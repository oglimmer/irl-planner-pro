...
DEFAULT_FRONTEND_DEPLOYMENT="irl-planner-pro-frontend"
DEFAULT_BACKEND_DEPLOYMENT="irl-planner-pro-backend"
...
RESTART_NAMESPACE="${RESTART_NAMESPACE:-irl-planner-pro}"
HELP=false
...
Build, deploy, and release irl-planner-pro application components.
...
ENVIRONMENT VARIABLES:
    ...
    RESTART_NAMESPACE       Namespace for the restart hook (default: irl-planner-pro)
...
parse_args() {
    ...
    if [[ ${#REGISTRIES[@]} -gt 0 ]]; then
        ...
        for registry in "${REGISTRIES[@]}"; do
            FRONTEND_IMAGES+=("$registry/irl-planner-pro-frontend")
            BACKEND_IMAGES+=("$registry/irl-planner-pro-backend")
        done
    else
        # Fallback to defaults if no registries specified
        FRONTEND_IMAGES=("${DEFAULT_REGISTRIES[0]}/irl-planner-pro-frontend")
        BACKEND_IMAGES=("${DEFAULT_REGISTRIES[0]}/irl-planner-pro-backend")
    fi
...
# without manual setup. The in-repo default secret is rejected at startup, so a
# fresh clone can't `start` otherwise. AUTH_MODE=password skips the OIDC setup
# for local dev. Only written when missing — an existing .env is never touched.
ensure_backend_env() {
    ...
    log_info "Created $BACKEND_DIR/.env with a generated JWT_SECRET and AUTH_MODE=password (local dev only)."
    log_info "The backend still needs DATABASE_URL pointing at a running Postgres (see compose.yml)."
}
...
cmd_helm_push() {
    local chart_dir="$SCRIPT_DIR/helm/irl-planner-pro"
    ...
    if helm push "$tmp_dir/irl-planner-pro-${chart_version}.tgz" "oci://$registry"; then
        log_success "Helm chart v$chart_version pushed to oci://$registry/irl-planner-pro"
    else
...
restart_via_kubectl() {
    ...
    if execute_cmd "kubectl rollout restart deployment/$deployment -n $RESTART_NAMESPACE"; then
        log_success "Deployment $deployment restarted successfully"

        # Wait for rollout to complete if verbose
        if [[ "$VERBOSE" == true ]]; then
            log_info "Waiting for rollout to complete..."
            kubectl rollout status deployment/"$deployment" -n "$RESTART_NAMESPACE" --timeout=300s
        fi
    else
...
execute_release() {
    ...
    local chart_file="$SCRIPT_DIR/helm/irl-planner-pro/Chart.yaml"
