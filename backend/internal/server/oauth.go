...
// OAuth 2.1 (Authorization Code + PKCE) for the Phase 7 MCP endpoint. The whole
// surface is inert unless MCP_OAUTH_CLIENT_ID/SECRET are configured: every
// handler 404s when a.Cfg.MCPOAuthClientID == "", and router.go only mounts the
// routes when MCPEnabled(). Adapted from the plugin-skill-hosting reference,
// trimmed of its password/bcrypt and user-status concepts.

// oauthRealm labels the Bearer/Basic challenges for this deployment.
const oauthRealm = `Bearer realm="irl-planner"`

// loginFormTmpl is rendered by GET /oauth/authorize in password mode. AUTH_MODE
// =password is a local-dev stub with no real credential check (mirroring
// handleDevLogin), so the form asks only for an email and mints a session for
// whatever address is entered. Never enable password mode on a shared deployment.
...
h1{font-size:1.15rem;margin:0 0 .5rem;font-weight:600;color:#111}
p.sub{font-size:.85rem;color:#6b7280;margin:0 0 1.5rem}
...
<p class="sub">Development sign-in — any email is accepted.</p>
...
// (RFC 9728), advertising which authorization server protects /mcp. Registered
// on both the bare path and the /mcp-suffixed path so clients that derive the
// metadata URL from the resource identifier and those that read the /mcp 401
...
// pointer; otherwise it falls back to the bare Bearer challenge.
func (a *App) mcpAuthChallenge() string {
	if a.Cfg.MCPOAuthClientID == "" {
		return oauthRealm
	}
	base := strings.TrimRight(a.Cfg.PublicBaseURL, "/")
	return fmt.Sprintf("%s, resource_metadata=%q", oauthRealm,
		base+"/.well-known/oauth-protected-resource/mcp")
}

// handleOAuthAuthorize serves GET /oauth/authorize. In oidc mode it stashes the
// OAuth params in oauth_pending and redirects to the OIDC provider (the OIDC
// callback resumes the flow). In password mode it renders the dev login form.
func (a *App) handleOAuthAuthorize(w http.ResponseWriter, r *http.Request) {
	...
	// Validate client_id and redirect_uri before redirecting — prevents an
	// open-redirect via a forged redirect_uri.
	...
	// Password (dev) mode: render the email-only login form.
...
// The dev login form posts here; on success it redirects to the OAuth client's
// redirect_uri with an authorization code. No credential is checked — this
// mirrors handleDevLogin and is for local development only.
func (a *App) handleOAuthAuthorizeSubmit(w http.ResponseWriter, r *http.Request) {
	...
	if email == "" || !strings.Contains(email, "@") {
		rerender("A valid email is required.")
		return
	}
	...
	authCode, err := a.issueAuthCode(r.Context(), user.ID, redirectURI, codeChallenge)
	if err != nil {
...
// handleOAuthToken serves POST /oauth/token (authorization_code + refresh_token).
...
func (a *App) handleRefreshToken(w http.ResponseWriter, r *http.Request) {
	...
	// Rotate: atomically delete the old token and read the user_id. The join to
	// users enforces that the account still exists before a fresh pair is minted.
	var userID string
	err := a.DB.QueryRowContext(r.Context(),
...
func (a *App) issueOAuthTokenPair(ctx context.Context, userID string) (*oauthTokenPair, error) {
	...
	// re-issued one with the new version).
...
// context stored while the user was going through the OIDC leg. Returns
// (nil, nil) when the row is not found or has expired.
...
// StartOAuthGC launches a background goroutine that periodically deletes expired
// rows from the OAuth tables. The happy path already deletes rows on successful
// exchange/rotation, but abandoned flows (user closes the tab, client crashes)
// leave dead rows that the expires_at guards keep out of use but never remove.
...
func (a *App) validateOAuthClient(r *http.Request) bool {
	...
	// circuit) so the response time doesn't reveal which field mismatched.
...
// error query parameters. Used only after the redirect_uri is known-valid.
...
// merging them into any query string the URI already carries. redirect_uri is
// exact-match validated against the allowlist before reaching here, so a parse
// failure is not reachable in practice — fall back to the naive form rather than
// dropping the redirect entirely.
