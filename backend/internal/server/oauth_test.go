...
// access — the happy-path code exchange and refresh rotation are the only paths
// that touch Postgres and are out of scope here.
func oauthTestApp() *App {
	return &App{Cfg: config.Config{
		JWTSecret:            "test-secret-at-least-32-characters-long",
		PublicBaseURL:        "https://irl.example.com",
		AuthMode:             "password",
...
func TestHandleOAuthMeta(t *testing.T) {
	...
	if body["issuer"] != "https://irl.example.com" {
		t.Errorf("issuer = %v", body["issuer"])
	}
	if body["authorization_endpoint"] != "https://irl.example.com/oauth/authorize" {
		t.Errorf("authorization_endpoint = %v", body["authorization_endpoint"])
	}
	if body["token_endpoint"] != "https://irl.example.com/oauth/token" {
		t.Errorf("token_endpoint = %v", body["token_endpoint"])
	}
}

func TestHandleOAuthMeta_DisabledIs404(t *testing.T) {
	a := &App{Cfg: config.Config{PublicBaseURL: "https://irl.example.com"}} // no client id
	rec := httptest.NewRecorder()
	a.handleOAuthMeta(rec, httptest.NewRequest("GET", "/.well-known/oauth-authorization-server", nil))
...
func TestMCPAuthChallenge(t *testing.T) {
	...
	if !strings.Contains(got, `resource_metadata="https://irl.example.com/.well-known/oauth-protected-resource/mcp"`) {
		t.Errorf("challenge missing resource_metadata pointer: %q", got)
	}
	if !strings.HasPrefix(got, oauthRealm) {
		t.Errorf("challenge missing Bearer realm: %q", got)
	}
}

func TestMCPAuthChallenge_DisabledIsBareBearer(t *testing.T) {
	a := &App{Cfg: config.Config{PublicBaseURL: "https://irl.example.com"}}
	if got := a.mcpAuthChallenge(); got != oauthRealm {
...
func TestHandleOAuthAuthorize_ValidRendersLoginForm(t *testing.T) {
	a := oauthTestApp() // password mode → email-only dev form
	rec := httptest.NewRecorder()
...
func TestResolveTokenRejectsMCPScopeOutsideMCP(t *testing.T) {
	...
	// allowMCPScope=false (the /api gate) must reject the token before any DB
	// lookup, so a nil DB never gets touched.
...
func TestDiscoveryRoutesResolveWhenMCPEnabled(t *testing.T) {
...
func TestMCPRoutesAbsentWhenDisabled(t *testing.T) {
	// No MCP_OAUTH_* → the whole surface is unmounted, so even discovery 404s.
	...
	for _, path := range []string{
		...
		router.ServeHTTP(rec, httptest.NewRequest("GET", path, nil))
		if rec.Code != http.StatusNotFound {
			t.Errorf("GET %s = %d, want 404 when MCP disabled", path, rec.Code)
		}
	}
}

func TestMCPEndpointUnauthenticatedChallenge(t *testing.T) {
	// With OAuth configured, /mcp without a bearer token must 401 and advertise
	// the resource-metadata pointer so clients can discover the OAuth server.
	router := NewRouter(oauthTestApp())
	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, httptest.NewRequest("POST", "/mcp", strings.NewReader("{}")))
	if rec.Code != http.StatusUnauthorized {
		t.Fatalf("status = %d, want 401", rec.Code)
	}
	if ch := rec.Header().Get("WWW-Authenticate"); !strings.Contains(ch, "resource_metadata=") {
		t.Errorf("WWW-Authenticate = %q, want resource_metadata pointer", ch)
	}
}

// The OAuth token endpoint must shed load past its per-IP rate limit. Every
// request fails client validation (no creds) and returns before any DB access,
// but the limiter counts each one, so a burst from one IP eventually trips 429.
