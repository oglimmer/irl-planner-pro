...
// both win). On subsequent logins the display name is refreshed from the IdP.
func (a *App) findOrCreateUser(ctx context.Context, email, name string) (*User, error) {
	email = strings.ToLower(strings.TrimSpace(email))
	name = strings.TrimSpace(name)

	u := &User{}
	err := a.DB.QueryRowContext(ctx,
		`SELECT id, email, name, is_admin, created_at, token_version FROM users WHERE email = $1`, email).
		Scan(&u.ID, &u.Email, &u.Name, &u.IsAdmin, &u.CreatedAt, &u.TokenVersion)
	if err == nil {
		// Refresh the display name if the IdP supplied a non-empty one.
		if name != "" && name != u.Name {
			if _, e := a.DB.ExecContext(ctx, `UPDATE users SET name = $1 WHERE id = $2`, name, u.ID); e == nil {
				u.Name = name
			}
		}
		return u, nil
	...
	err = a.DB.QueryRowContext(ctx,
		`INSERT INTO users (email, name, is_admin)
		 ...
		 RETURNING id, email, name, is_admin, created_at, token_version`,
		email, name).
		Scan(&u.ID, &u.Email, &u.Name, &u.IsAdmin, &u.CreatedAt, &u.TokenVersion)
	if err != nil {
		return nil, err
...
// UserSummary is the admin user-list row.
...
func (a *App) handleListUsers(w http.ResponseWriter, r *http.Request) {
	rows, err := a.DB.QueryContext(r.Context(),
		`SELECT id, email, name, is_admin, created_at FROM users ORDER BY created_at`)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&u.ID, &u.Email, &u.Name, &u.IsAdmin, &created); err != nil {
			serverErr(w, r, err, "db error")
			return
		}
		if created.Valid {
...
func (a *App) handleDevLogin(w http.ResponseWriter, r *http.Request) {
	...
	user, err := a.findOrCreateUser(r.Context(), req.Email, req.Name)
	if err != nil {
		serverErr(w, r, err, "db error")
		return
	}
