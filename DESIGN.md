# IRL Attendance App — Architecture & Reference
...
| Auth | **OIDC** (`coreos/go-oidc/v3`) + **JWT** sessions (`golang-jwt/jwt/v5`) | Google Workspace `hd`-claim domain restriction → `oglimmer.com` |
...
- **Three deployment profiles** (see `.env.example`):
  - **prod** — `AUTH_MODE=oidc`, Google as the OIDC provider, restricted to oglimmer.com.
  ...
  - **local** — `AUTH_MODE=password`, a dev stub that mints a session for any email
    ...
    deployment.
- In prod, `OIDC_GOOGLE_WORKSPACE_DOMAINS=oglimmer.com` enforces the `hd` claim via
...
# Auth — OIDC. Google Workspace, restricted to oglimmer.com.
AUTH_MODE=oidc
OIDC_ISSUER_URL=https://accounts.google.com
OIDC_CLIENT_ID=...
OIDC_CLIENT_SECRET=...
OIDC_REDIRECT_URL=                       # defaults to PUBLIC_BASE_URL + /api/auth/oidc/callback
OIDC_GOOGLE_WORKSPACE_DOMAINS=oglimmer.com
